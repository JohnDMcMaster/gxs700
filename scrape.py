...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Convert USB .cap into gxs700 API calls')
    ...
    if args.fin.find('.cap') >= 0 or args.fin.find(
            '.pcap') >= 0 or args.fin.find('.pcapng') >= 0:
        ...
        if args.device:
            ...
            RESET_FILTER = 0
        cmd = '%s --packet-numbers --no-setup --comment --fx2 -j %s %s >%s' % (
            args.usbrply, device_arg, args.fin, fin_j)
        print cmd
